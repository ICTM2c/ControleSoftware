...
public class DbProduct extends Db {
     ...
     * @throws SQLException
     */
    public List<Models.Product> findProductsForOrder(int order) throws SQLException, OrderNotFoundException {
        ...
        PreparedStatement stmt = getConnection().prepareStatement("SELECT p.*, s.* FROM Product p\n" +
                "INNER JOIN product_order o ON p.productid = o.ProductId\n" +
        ...
        List<Models.Product> res = new ArrayList<>();
        while (rs.next()) {
            res.add(Models.Product.fromResultSet(rs));
        }
        stmt.close();
